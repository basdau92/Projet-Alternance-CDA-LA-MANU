...
class ClientController extends Controller
{
    ...
    public function upload(Request $request)
    {
        if (!$request->hasFile('document')) { // ensure the request has a file
            return response()->json('Aucun fichier n\'a été trovué!', 400); // client side error, Bad Request
        }
    ...
    /**
     * Retrieve all the documents by client's id
     */
     ...
     */
    public function readSingleDocument($id)
    {
        try {
            $document = ClientDocument::with((['showDocument']))
                ->where('id_client', Auth::userOrFail()->id)
                ->get();
            $document = ClientDocument::findOrFail($id); // retrieve A document from its id
            ...
            return response()->json(['message' => 'Le document n\'a pas été trouvé!'], 404);
        }
    }
